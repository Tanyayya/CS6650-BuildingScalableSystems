...
// 💥 THE CRASH
// Sleeps 10-30 seconds on every request.
// Under load, goroutines pile up in memory.
// Response times hit 30,000ms. System collapses.
func getRecommendations(category string) []string {
...
func handleSearch(w http.ResponseWriter, r *http.Request, store *Store) {
	...
	var recs []string
	if len(products) > 0 {
		recs = getRecommendations(products[0].Category) // 💥 no timeout, blocks forever
	}
...
func main() {
	...
	log.Println("💥 CRASH VERSION — recommendation service will cause failures under load")
