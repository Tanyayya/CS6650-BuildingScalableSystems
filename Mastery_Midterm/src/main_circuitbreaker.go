...
type SearchResponse struct {
	...
	RecSource       string    `json:"rec_source"` // "live", "timeout", "circuit_open"
}
...
//   OPEN    → broken, calls blocked for 30s
//   HALF    → cooldown passed, try again
//
// After 5 consecutive timeouts → OPEN
// After 30s cooldown → CLOSED and retry
// ─────────────────────────────────────────
...
func (c *CircuitBreaker) IsOpen() bool {
	...
	if c.failures >= c.maxFailures {
		...
		log.Println("🔄 Circuit breaker: cooldown passed, resetting")
		c.failures = 0
	}
	return false // CLOSED
}
...
func (c *CircuitBreaker) RecordFailure() {
	...
	log.Printf("⚠️  Circuit breaker: failure %d/%d", c.failures, c.maxFailures)
	if c.failures >= c.maxFailures {
		log.Println("🔴 Circuit breaker OPEN — blocking calls for 30s")
	}
}
...
func (c *CircuitBreaker) RecordSuccess() {
	...
	defer c.mu.Unlock()
	c.failures = 0
	log.Println("✅ Circuit breaker: success, staying CLOSED")
}

func (c *CircuitBreaker) Status() string {
	c.mu.Lock()
	defer c.mu.Unlock()
	if c.failures >= c.maxFailures {
		remaining := c.cooldown - time.Since(c.lastFailure)
		return fmt.Sprintf("OPEN (resets in %.0fs)", remaining.Seconds())
	}
	return fmt.Sprintf("CLOSED (failures=%d/%d)", c.failures, c.maxFailures)
}

func getRecsCircuitBreaker(category string) ([]string, string) {
	if cb.IsOpen() {
		log.Printf("🔴 Circuit OPEN — skipping recommendation call")
		return []string{}, "circuit_open"
	}
...
func handleSearch(w http.ResponseWriter, r *http.Request, store *Store) {
	...
	if len(products) > 0 {
		recs, recSource = getRecsCircuitBreaker(products[0].Category)
	} else {
...
func main() {
	...
	log.Println("✅ FIX 2: CIRCUIT BREAKER — 5 failures → OPEN for 30s")
